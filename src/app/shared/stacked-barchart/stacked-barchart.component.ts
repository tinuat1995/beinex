...
private initSvg() {
    this.svg = d3.select('#chartdata').append('svg').attr('width',350).attr('height',300);
...
private drawChart(data: any[]) {
    ...
    this.y.domain(data.map((d: any) => d.State));
    ...
    this.z.domain(keys);
        ...
        .data((d:any) => d)
        .enter().append('rect')
        .attr('y', (d:any) => this.y(d.data.State))
    ...
    // const group = bar.selectAll("text")
    // .data((d:any) => this.x(d[0]))
    // .enter()
    // .append("text")
